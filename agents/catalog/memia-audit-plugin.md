---
id: memia-audit-plugin
name: MEMIA Audit Plugin
product: MEMIA BPOS
status: included
...
Aide à cadrer les audits opérationnels et fournit une synthèse actionnable avec priorités, arbitrages, risques et prochaines actions.

## FR - Fiche agent

### Fonction

MEMIA Audit Plugin intervient quand une demande touche les audits opérationnels. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les audits opérationnels ;
...
MEMIA Audit Plugin is used when a request involves Audit Plugin. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Audit Plugin;
...
`memia agents show memia-audit-plugin`
