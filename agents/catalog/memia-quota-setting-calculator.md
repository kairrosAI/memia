---
id: memia-quota-setting-calculator
name: MEMIA Quota Setting Calculator
product: MEMIA BPOS
status: included
...
# MEMIA Quota Setting Calculator

Aide à cadrer la définition de quotas et fournit un plan de contenu ou d’activation avec audience, message, canaux, variantes et mesure.

## FR - Fiche agent

### Fonction

MEMIA Quota Setting Calculator intervient quand une demande touche la définition de quotas. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne la définition de quotas ;
...
MEMIA Quota Setting Calculator is used when a request involves Quota Setting Calculator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Quota Setting Calculator;
...
`memia agents show memia-quota-setting-calculator`
