---
id: memia-roi-calculator
name: MEMIA ROI Calculator
product: MEMIA BPOS
status: included
...
# MEMIA ROI Calculator

Aide à cadrer les calculs de ROI et fournit un cadre clair avec angle, contraintes, critères de qualité et éléments à valider.

## FR - Fiche agent

### Fonction

MEMIA ROI Calculator intervient quand une demande touche les calculs de ROI. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les calculs de ROI ;
...
MEMIA ROI Calculator is used when a request involves ROI Calculator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions ROI Calculator;
...
`memia agents show memia-roi-calculator`
