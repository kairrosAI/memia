---
id: memia-go-handler-generator
name: MEMIA Go Handler Generator
product: MEMIA BPOS
status: included
...
# MEMIA Go Handler Generator

Produit les handlers Go et fournit une proposition technique avec structure, contraintes, tests, risques et critères de validation.

## FR - Fiche agent

### Fonction

MEMIA Go Handler Generator intervient quand une demande touche les handlers Go. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les handlers Go ;
...
MEMIA Go Handler Generator is used when a request involves Go Handler Generator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Go Handler Generator;
...
`memia agents show memia-go-handler-generator`
