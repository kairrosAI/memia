---
id: memia-pwa-manifest-generator
name: MEMIA PWA Manifest Generator
product: MEMIA BPOS
status: included
...
# MEMIA PWA Manifest Generator

Produit les manifests PWA et fournit une proposition technique avec structure, contraintes, tests, risques et critères de validation.

## FR - Fiche agent

### Fonction

MEMIA PWA Manifest Generator intervient quand une demande touche les manifests PWA. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les manifests PWA ;
...
MEMIA PWA Manifest Generator is used when a request involves PWA Manifest Generator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions PWA Manifest Generator;
...
`memia agents show memia-pwa-manifest-generator`
