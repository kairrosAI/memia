---
id: memia-excel-lbo-modeler
name: MEMIA Excel LBO Modeler
product: MEMIA BPOS
status: included
...
# MEMIA Excel LBO Modeler

Aide à cadrer les modèles LBO dans Excel et fournit une fiche exploitable avec objectifs, contexte, points ouverts, décisions à prendre et prochaines étapes.

## FR - Fiche agent

### Fonction

MEMIA Excel LBO Modeler intervient quand une demande touche les modèles LBO dans Excel. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les modèles LBO dans Excel ;
...
MEMIA Excel LBO Modeler is used when a request involves Excel LBO Modeler. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Excel LBO Modeler;
...
`memia agents show memia-excel-lbo-modeler`
