---
id: memia-sync-status
name: MEMIA Sync Status
product: MEMIA BPOS
status: included
...
Prépare le statut de synchronisation et fournit une synthèse actionnable avec priorités, arbitrages, risques et prochaines actions.

## FR - Fiche agent

### Fonction

MEMIA Sync Status intervient quand une demande touche le statut de synchronisation. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne le statut de synchronisation ;
...
MEMIA Sync Status is used when a request involves Sync Status. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Sync Status;
...
`memia agents show memia-sync-status`
