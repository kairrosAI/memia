---
id: memia-aggregation-helper
name: MEMIA Aggregation Helper
product: MEMIA BPOS
status: included
...
# MEMIA Aggregation Helper

Aide à cadrer les agrégations et fournit une analyse vérifiable avec sources, métriques, hypothèses, limites et recommandations.

## FR - Fiche agent

### Fonction

MEMIA Aggregation Helper intervient quand une demande touche les agrégations. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les agrégations ;
...
MEMIA Aggregation Helper is used when a request involves Aggregation Helper. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Aggregation Helper;
...
`memia agents show memia-aggregation-helper`
