---
id: memia-airflow-dag-generator
name: MEMIA Airflow DAG Generator
product: MEMIA BPOS
status: included
...
# MEMIA Airflow DAG Generator

Produit les DAG Airflow et fournit une analyse vérifiable avec sources, métriques, hypothèses, limites et recommandations.

## FR - Fiche agent

### Fonction

MEMIA Airflow DAG Generator intervient quand une demande touche les DAG Airflow. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les DAG Airflow ;
...
MEMIA Airflow DAG Generator is used when a request involves Airflow DAG Generator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Airflow DAG Generator;
...
`memia agents show memia-airflow-dag-generator`
