---
id: memia-linear-issue-generator
name: MEMIA Linear Issue Generator
product: MEMIA BPOS
status: included
...
# MEMIA Linear Issue Generator

Produit les tickets produit et fournit une fiche exploitable avec objectifs, contexte, points ouverts, décisions à prendre et prochaines étapes.

## FR - Fiche agent

### Fonction

MEMIA Linear Issue Generator intervient quand une demande touche les tickets produit. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les tickets produit ;
...
MEMIA Linear Issue Generator is used when a request involves Linear Issue Generator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Linear Issue Generator;
...
`memia agents show memia-linear-issue-generator`
