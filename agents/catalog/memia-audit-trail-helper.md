---
id: memia-audit-trail-helper
name: MEMIA Audit Trail Helper
product: MEMIA BPOS
status: included
...
# MEMIA Audit Trail Helper

Aide à cadrer les traces d’audit et fournit une fiche exploitable avec objectifs, contexte, points ouverts, décisions à prendre et prochaines étapes.

## FR - Fiche agent

### Fonction

MEMIA Audit Trail Helper intervient quand une demande touche les traces d’audit. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les traces d’audit ;
...
MEMIA Audit Trail Helper is used when a request involves Audit Trail Helper. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Audit Trail Helper;
...
`memia agents show memia-audit-trail-helper`
