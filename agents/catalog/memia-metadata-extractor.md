---
id: memia-metadata-extractor
name: MEMIA Metadata Extractor
product: MEMIA BPOS
status: included
...
# MEMIA Metadata Extractor

Collecte et organise l’extraction de métadonnées et fournit une analyse vérifiable avec sources, métriques, hypothèses, limites et recommandations.

## FR - Fiche agent

### Fonction

MEMIA Metadata Extractor intervient quand une demande touche l’extraction de métadonnées. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne l’extraction de métadonnées ;
...
MEMIA Metadata Extractor is used when a request involves Metadata Extractor. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Metadata Extractor;
...
`memia agents show memia-metadata-extractor`
