---
id: memia-utm-parameter-generator
name: MEMIA UTM Parameter Generator
product: MEMIA BPOS
status: included
...
# MEMIA UTM Parameter Generator

Produit les paramètres UTM et fournit une analyse vérifiable avec sources, métriques, hypothèses, limites et recommandations.

## FR - Fiche agent

### Fonction

MEMIA UTM Parameter Generator intervient quand une demande touche les paramètres UTM. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les paramètres UTM ;
...
MEMIA UTM Parameter Generator is used when a request involves UTM Parameter Generator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions UTM Parameter Generator;
...
`memia agents show memia-utm-parameter-generator`
