---
id: memia-technical-writer
name: MEMIA Technical Writer
product: MEMIA BPOS
status: included
...
# MEMIA Technical Writer

Rédige la documentation technique et fournit un cadre clair avec angle, contraintes, critères de qualité et éléments à valider.

## FR - Fiche agent

### Fonction

MEMIA Technical Writer intervient quand une demande touche la documentation technique. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne la documentation technique ;
...
MEMIA Technical Writer is used when a request involves Technical Writer. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Technical Writer;
...
`memia agents show memia-technical-writer`
