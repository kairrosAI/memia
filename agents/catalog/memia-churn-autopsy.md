---
id: memia-churn-autopsy
name: MEMIA Churn Autopsy
product: MEMIA BPOS
status: included
...
Analyse l’analyse du churn et fournit un cadrage produit avec problème, utilisateurs, options, métriques et décisions.

## FR - Fiche agent

### Fonction

MEMIA Churn Autopsy intervient quand une demande touche l’analyse du churn. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne l’analyse du churn ;
...
MEMIA Churn Autopsy is used when a request involves Churn Autopsy. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Churn Autopsy;
...
`memia agents show memia-churn-autopsy`
