---
id: memia-budget-optimizer
name: MEMIA Budget Optimizer
product: MEMIA BPOS
status: included
...
# MEMIA Budget Optimizer

Structure l’optimisation budgétaire et fournit un modèle de décision avec hypothèses, scénarios, limites et recommandations.

## FR - Fiche agent

### Fonction

MEMIA Budget Optimizer intervient quand une demande touche l’optimisation budgétaire. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne l’optimisation budgétaire ;
...
MEMIA Budget Optimizer is used when a request involves Budget Optimizer. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Budget Optimizer;
...
`memia agents show memia-budget-optimizer`
