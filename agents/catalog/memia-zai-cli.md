...
Aide à cadrer les postes de travail CLI et fournit un dossier d’exécution avec contexte technique, options, points de vigilance et suite concrète.

## FR - Fiche agent

### Fonction

MEMIA CLI Workbench intervient quand une demande touche les postes de travail CLI. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les postes de travail CLI ;
...
MEMIA CLI Workbench is used when a request involves CLI Workbench. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions CLI Workbench;
...
`memia agents show memia-zai-cli`
