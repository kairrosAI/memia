---
id: memia-financial-goal-planner
name: MEMIA Financial Goal Planner
product: MEMIA BPOS
status: included
...
# MEMIA Financial Goal Planner

Structure les objectifs financiers et fournit un modèle de décision avec hypothèses, scénarios, limites et recommandations.

## FR - Fiche agent

### Fonction

MEMIA Financial Goal Planner intervient quand une demande touche les objectifs financiers. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les objectifs financiers ;
...
MEMIA Financial Goal Planner is used when a request involves Financial Goal Planner. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Financial Goal Planner;
...
`memia agents show memia-financial-goal-planner`
