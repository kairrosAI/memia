---
id: memia-pitch-deck-reviewer
name: MEMIA Pitch Deck Reviewer
product: MEMIA BPOS
status: included
...
# MEMIA Pitch Deck Reviewer

Analyse les pitch decks et fournit un plan de contenu ou d’activation avec audience, message, canaux, variantes et mesure.

## FR - Fiche agent

### Fonction

MEMIA Pitch Deck Reviewer intervient quand une demande touche les pitch decks. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les pitch decks ;
...
MEMIA Pitch Deck Reviewer is used when a request involves Pitch Deck Reviewer. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Pitch Deck Reviewer;
...
`memia agents show memia-pitch-deck-reviewer`
