...
Prépare la configuration de synchronisation et fournit une synthèse actionnable avec priorités, arbitrages, risques et prochaines actions.

## FR - Fiche agent

### Fonction

MEMIA Sync Setup intervient quand une demande touche la configuration de synchronisation. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne la configuration de synchronisation ;
...
MEMIA Sync Setup is used when a request involves Sync Setup. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Sync Setup;
...
`memia agents show memia-sync-setup`
