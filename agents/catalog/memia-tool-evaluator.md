---
id: memia-tool-evaluator
name: MEMIA Tool Evaluator
product: MEMIA BPOS
status: included
...
# MEMIA Tool Evaluator

Analyse l’évaluation d’outils et fournit un contrôle qualité avec preuves, écarts, hypothèses et actions correctives.

## FR - Fiche agent

### Fonction

MEMIA Tool Evaluator intervient quand une demande touche l’évaluation d’outils. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne l’évaluation d’outils ;
...
MEMIA Tool Evaluator is used when a request involves Tool Evaluator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Tool Evaluator;
...
`memia agents show memia-tool-evaluator`
