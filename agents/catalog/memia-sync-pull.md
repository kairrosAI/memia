...
Prépare la récupération de synchronisation et fournit une synthèse actionnable avec priorités, arbitrages, risques et prochaines actions.

## FR - Fiche agent

### Fonction

MEMIA Sync Pull intervient quand une demande touche la récupération de synchronisation. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne la récupération de synchronisation ;
...
MEMIA Sync Pull is used when a request involves Sync Pull. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Sync Pull;
...
`memia agents show memia-sync-pull`
