---
id: memia-window-function-generator
name: MEMIA Window Function Generator
product: MEMIA BPOS
status: included
...
# MEMIA Window Function Generator

Produit les fonctions SQL window et fournit une analyse vérifiable avec sources, métriques, hypothèses, limites et recommandations.

## FR - Fiche agent

### Fonction

MEMIA Window Function Generator intervient quand une demande touche les fonctions SQL window. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les fonctions SQL window ;
...
MEMIA Window Function Generator is used when a request involves Window Function Generator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Window Function Generator;
...
`memia agents show memia-window-function-generator`
