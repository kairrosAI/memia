...
Aide à cadrer la mémoire de travail et fournit une synthèse actionnable avec priorités, arbitrages, risques et prochaines actions.

## FR - Fiche agent

### Fonction

MEMIA Memory Kit intervient quand une demande touche la mémoire de travail. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne la mémoire de travail ;
...
MEMIA Memory Kit is used when a request involves Memory Kit. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Memory Kit;
...
`memia agents show memia-memory-kit`
