---
id: memia-luigi-task-generator
name: MEMIA Luigi Task Generator
product: MEMIA BPOS
status: included
...
# MEMIA Luigi Task Generator

Produit les tâches Luigi et fournit une analyse vérifiable avec sources, métriques, hypothèses, limites et recommandations.

## FR - Fiche agent

### Fonction

MEMIA Luigi Task Generator intervient quand une demande touche les tâches Luigi. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les tâches Luigi ;
...
MEMIA Luigi Task Generator is used when a request involves Luigi Task Generator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Luigi Task Generator;
...
`memia agents show memia-luigi-task-generator`
