---
id: memia-lookalike-customer-finder
name: MEMIA Lookalike Customer Finder
product: MEMIA BPOS
status: included
...
# MEMIA Lookalike Customer Finder

Collecte et organise la recherche de clients similaires et fournit un cadre clair avec angle, contraintes, critères de qualité et éléments à valider.

## FR - Fiche agent

### Fonction

MEMIA Lookalike Customer Finder intervient quand une demande touche la recherche de clients similaires. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne la recherche de clients similaires ;
...
MEMIA Lookalike Customer Finder is used when a request involves Lookalike Customer Finder. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Lookalike Customer Finder;
...
`memia agents show memia-lookalike-customer-finder`
