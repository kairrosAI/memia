---
id: memia-firebase-vertex-ai
name: MEMIA AI Runtime Connector
product: MEMIA BPOS
status: included
...
# MEMIA AI Runtime Connector

Aide à cadrer les connecteurs runtime IA et fournit un dossier d’exécution avec contexte technique, options, points de vigilance et suite concrète.

## FR - Fiche agent

### Fonction

MEMIA AI Runtime Connector intervient quand une demande touche les connecteurs runtime IA. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les connecteurs runtime IA ;
...
MEMIA AI Runtime Connector is used when a request involves AI runtime connectors. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions AI runtime connectors;
...
`memia agents show memia-firebase-vertex-ai`
