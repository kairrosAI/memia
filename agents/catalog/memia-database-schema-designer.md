---
id: memia-database-schema-designer
name: MEMIA Database Schema Designer
product: MEMIA BPOS
status: included
...
# MEMIA Database Schema Designer

Produit les schémas de base de données et fournit une proposition technique avec structure, contraintes, tests, risques et critères de validation.

## FR - Fiche agent

### Fonction

MEMIA Database Schema Designer intervient quand une demande touche les schémas de base de données. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les schémas de base de données ;
...
MEMIA Database Schema Designer is used when a request involves Database Schema Designer. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Database Schema Designer;
...
`memia agents show memia-database-schema-designer`
