---
id: memia-agent-context-loader
name: MEMIA Agent Context Loader
product: MEMIA BPOS
status: included
...
# MEMIA Agent Context Loader

Prépare le chargement de contexte agent et fournit une synthèse actionnable avec priorités, arbitrages, risques et prochaines actions.

## FR - Fiche agent

### Fonction

MEMIA Agent Context Loader intervient quand une demande touche le chargement de contexte agent. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne le chargement de contexte agent ;
...
MEMIA Agent Context Loader is used when a request involves Agent Context Loader. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Agent Context Loader;
...
`memia agents show memia-agent-context-loader`
