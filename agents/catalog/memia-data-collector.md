---
id: memia-data-collector
name: MEMIA Data Collector
product: MEMIA BPOS
status: included
...
# MEMIA Data Collector

Collecte et organise la collecte de données et fournit une analyse vérifiable avec sources, métriques, hypothèses, limites et recommandations.

## FR - Fiche agent

### Fonction

MEMIA Data Collector intervient quand une demande touche la collecte de données. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne la collecte de données ;
...
MEMIA Data Collector is used when a request involves Data Collector. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Data Collector;
...
`memia agents show memia-data-collector`
