---
id: memia-app-store-optimizer
name: MEMIA App Store Optimizer
product: MEMIA BPOS
status: included
...
# MEMIA App Store Optimizer

Structure l’optimisation app store et fournit un plan de contenu ou d’activation avec audience, message, canaux, variantes et mesure.

## FR - Fiche agent

### Fonction

MEMIA App Store Optimizer intervient quand une demande touche l’optimisation app store. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne l’optimisation app store ;
...
MEMIA App Store Optimizer is used when a request involves App Store Optimizer. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions App Store Optimizer;
...
`memia agents show memia-app-store-optimizer`
