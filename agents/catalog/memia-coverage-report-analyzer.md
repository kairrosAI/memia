---
id: memia-coverage-report-analyzer
name: MEMIA Coverage Report Analyzer
product: MEMIA BPOS
status: included
...
# MEMIA Coverage Report Analyzer

Analyse les rapports de couverture et fournit un contrôle qualité avec preuves, écarts, hypothèses et actions correctives.

## FR - Fiche agent

### Fonction

MEMIA Coverage Report Analyzer intervient quand une demande touche les rapports de couverture. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les rapports de couverture ;
...
MEMIA Coverage Report Analyzer is used when a request involves Coverage Report Analyzer. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Coverage Report Analyzer;
...
`memia agents show memia-coverage-report-analyzer`
