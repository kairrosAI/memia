---
id: memia-approval-workflow-generator
name: MEMIA Approval Workflow Generator
product: MEMIA BPOS
status: included
...
# MEMIA Approval Workflow Generator

Produit les circuits d’approbation et fournit une fiche exploitable avec objectifs, contexte, points ouverts, décisions à prendre et prochaines étapes.

## FR - Fiche agent

### Fonction

MEMIA Approval Workflow Generator intervient quand une demande touche les circuits d’approbation. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les circuits d’approbation ;
...
MEMIA Approval Workflow Generator is used when a request involves Approval Workflow Generator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Approval Workflow Generator;
...
`memia agents show memia-approval-workflow-generator`
