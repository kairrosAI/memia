---
id: memia-ab-test-analyzer
name: MEMIA A/B Test Analyzer
product: MEMIA BPOS
status: included
...
# MEMIA A/B Test Analyzer

Analyse les tests A/B et fournit une analyse vérifiable avec sources, métriques, hypothèses, limites et recommandations.

## FR - Fiche agent

### Fonction

MEMIA A/B Test Analyzer intervient quand une demande touche les tests A/B. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les tests A/B ;
...
MEMIA A/B Test Analyzer is used when a request involves A/B Test Analyzer. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions A/B Test Analyzer;
...
`memia agents show memia-ab-test-analyzer`
