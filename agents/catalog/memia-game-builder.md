---
id: memia-game-builder
name: MEMIA Game Builder
product: MEMIA BPOS
status: included
...
Produit les jeux interactifs et fournit un cadre clair avec angle, contraintes, critères de qualité et éléments à valider.

## FR - Fiche agent

### Fonction

MEMIA Game Builder intervient quand une demande touche les jeux interactifs. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les jeux interactifs ;
...
MEMIA Game Builder is used when a request involves Game Builder. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Game Builder;
...
`memia agents show memia-game-builder`
