---
id: memia-accessibility-audit-runner
name: MEMIA Accessibility Audit Runner
product: MEMIA BPOS
status: included
...
# MEMIA Accessibility Audit Runner

Aide à cadrer les audits d’accessibilité et fournit une proposition technique avec structure, contraintes, tests, risques et critères de validation.

## FR - Fiche agent

### Fonction

MEMIA Accessibility Audit Runner intervient quand une demande touche les audits d’accessibilité. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les audits d’accessibilité ;
...
MEMIA Accessibility Audit Runner is used when a request involves Accessibility Audit Runner. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Accessibility Audit Runner;
...
`memia agents show memia-accessibility-audit-runner`
