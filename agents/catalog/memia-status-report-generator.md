---
id: memia-status-report-generator
name: MEMIA Status Report Generator
product: MEMIA BPOS
status: included
...
# MEMIA Status Report Generator

Produit les rapports de statut et fournit une fiche exploitable avec objectifs, contexte, points ouverts, décisions à prendre et prochaines étapes.

## FR - Fiche agent

### Fonction

MEMIA Status Report Generator intervient quand une demande touche les rapports de statut. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les rapports de statut ;
...
MEMIA Status Report Generator is used when a request involves Status Report Generator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Status Report Generator;
...
`memia agents show memia-status-report-generator`
