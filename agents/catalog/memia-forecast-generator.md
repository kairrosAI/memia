---
id: memia-forecast-generator
name: MEMIA Forecast Generator
product: MEMIA BPOS
status: included
...
# MEMIA Forecast Generator

Produit les prévisions et fournit une analyse vérifiable avec sources, métriques, hypothèses, limites et recommandations.

## FR - Fiche agent

### Fonction

MEMIA Forecast Generator intervient quand une demande touche les prévisions. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les prévisions ;
...
MEMIA Forecast Generator is used when a request involves Forecast Generator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Forecast Generator;
...
`memia agents show memia-forecast-generator`
