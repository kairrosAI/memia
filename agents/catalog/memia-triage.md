...
Aide à cadrer le triage opérationnel et fournit une synthèse actionnable avec priorités, arbitrages, risques et prochaines actions.

## FR - Fiche agent

### Fonction

MEMIA Triage intervient quand une demande touche le triage opérationnel. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne le triage opérationnel ;
...
MEMIA Triage is used when a request involves Triage. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Triage;
...
`memia agents show memia-triage`
