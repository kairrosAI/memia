---
id: memia-data-partitioner
name: MEMIA Data Partitioner
product: MEMIA BPOS
status: included
...
# MEMIA Data Partitioner

Aide à cadrer le partitionnement de données et fournit une analyse vérifiable avec sources, métriques, hypothèses, limites et recommandations.

## FR - Fiche agent

### Fonction

MEMIA Data Partitioner intervient quand une demande touche le partitionnement de données. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne le partitionnement de données ;
...
MEMIA Data Partitioner is used when a request involves Data Partitioner. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Data Partitioner;
...
`memia agents show memia-data-partitioner`
