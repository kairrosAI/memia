---
id: memia-deprecation-notice-generator
name: MEMIA Deprecation Notice Generator
product: MEMIA BPOS
status: included
...
# MEMIA Deprecation Notice Generator

Produit les avis de dépréciation et fournit une documentation claire avec public cible, structure, exemples et maintenance prévue.

## FR - Fiche agent

### Fonction

MEMIA Deprecation Notice Generator intervient quand une demande touche les avis de dépréciation. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les avis de dépréciation ;
...
MEMIA Deprecation Notice Generator is used when a request involves Deprecation Notice Generator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Deprecation Notice Generator;
...
`memia agents show memia-deprecation-notice-generator`
