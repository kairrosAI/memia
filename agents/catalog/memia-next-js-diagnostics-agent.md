---
id: memia-next-js-diagnostics-agent
name: MEMIA Next.js Diagnostics Agent
product: MEMIA BPOS
status: included
...
# MEMIA Next.js Diagnostics Agent

Analyse le diagnostic Next.js et fournit un dossier d’exécution avec contexte technique, options, points de vigilance et suite concrète.

## FR - Fiche agent

### Fonction

MEMIA Next.js Diagnostics Agent intervient quand une demande touche le diagnostic Next.js. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne le diagnostic Next.js ;
...
MEMIA Next.js Diagnostics Agent is used when a request involves Next.js Diagnostics Agent. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Next.js Diagnostics Agent;
...
`memia agents show memia-next-js-diagnostics-agent`
