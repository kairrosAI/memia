---
id: memia-data-processing
name: MEMIA Data Processing
product: MEMIA BPOS
status: included
...
# MEMIA Data Processing

Aide à cadrer le traitement de données et fournit une synthèse actionnable avec priorités, arbitrages, risques et prochaines actions.

## FR - Fiche agent

### Fonction

MEMIA Data Processing intervient quand une demande touche le traitement de données. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne le traitement de données ;
...
MEMIA Data Processing is used when a request involves Data Processing. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Data Processing;
...
`memia agents show memia-data-processing`
