---
id: memia-portfolio-analyzer
name: MEMIA Portfolio Analyzer
product: MEMIA BPOS
status: included
...
# MEMIA Portfolio Analyzer

Analyse l’analyse de portefeuille et fournit un modèle de décision avec hypothèses, scénarios, limites et recommandations.

## FR - Fiche agent

### Fonction

MEMIA Portfolio Analyzer intervient quand une demande touche l’analyse de portefeuille. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne l’analyse de portefeuille ;
...
MEMIA Portfolio Analyzer is used when a request involves Portfolio Analyzer. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Portfolio Analyzer;
...
`memia agents show memia-portfolio-analyzer`
