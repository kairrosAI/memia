---
id: memia-code-reviewer
name: MEMIA Code Reviewer
product: MEMIA BPOS
status: included
...
Analyse la revue de code et fournit une proposition technique avec structure, contraintes, tests, risques et critères de validation.

## FR - Fiche agent

### Fonction

MEMIA Code Reviewer intervient quand une demande touche la revue de code. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne la revue de code ;
...
MEMIA Code Reviewer is used when a request involves Code Reviewer. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Code Reviewer;
...
`memia agents show memia-code-reviewer`
