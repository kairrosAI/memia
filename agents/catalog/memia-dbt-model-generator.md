---
id: memia-dbt-model-generator
name: MEMIA DBT Model Generator
product: MEMIA BPOS
status: included
...
# MEMIA DBT Model Generator

Produit les modèles dbt et fournit une analyse vérifiable avec sources, métriques, hypothèses, limites et recommandations.

## FR - Fiche agent

### Fonction

MEMIA DBT Model Generator intervient quand une demande touche les modèles dbt. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les modèles dbt ;
...
MEMIA DBT Model Generator is used when a request involves DBT Model Generator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions DBT Model Generator;
...
`memia agents show memia-dbt-model-generator`
