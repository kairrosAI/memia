---
id: memia-regex-visual-debugger
name: MEMIA Regex Visual Debugger
product: MEMIA BPOS
status: included
...
# MEMIA Regex Visual Debugger

Aide à cadrer le débogage visuel de regex et fournit un cadre clair avec angle, contraintes, critères de qualité et éléments à valider.

## FR - Fiche agent

### Fonction

MEMIA Regex Visual Debugger intervient quand une demande touche le débogage visuel de regex. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne le débogage visuel de regex ;
...
MEMIA Regex Visual Debugger is used when a request involves Regex Visual Debugger. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Regex Visual Debugger;
...
`memia agents show memia-regex-visual-debugger`
