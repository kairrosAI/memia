---
id: memia-security-scanning
name: MEMIA Security Scanning
product: MEMIA BPOS
status: included
...
# MEMIA Security Scanning

Analyse la revue sécurité et fournit une synthèse actionnable avec priorités, arbitrages, risques et prochaines actions.

## FR - Fiche agent

### Fonction

MEMIA Security Scanning intervient quand une demande touche la revue sécurité. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne la revue sécurité ;
...
MEMIA Security Scanning is used when a request involves Security Scanning. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Security Scanning;
...
`memia agents show memia-security-scanning`
