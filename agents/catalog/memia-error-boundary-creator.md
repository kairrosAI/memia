---
id: memia-error-boundary-creator
name: MEMIA Error Boundary Creator
product: MEMIA BPOS
status: included
...
# MEMIA Error Boundary Creator

Produit la gestion des erreurs applicatives et fournit un cadre clair avec angle, contraintes, critères de qualité et éléments à valider.

## FR - Fiche agent

### Fonction

MEMIA Error Boundary Creator intervient quand une demande touche la gestion des erreurs applicatives. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne la gestion des erreurs applicatives ;
...
MEMIA Error Boundary Creator is used when a request involves Error Boundary Creator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions Error Boundary Creator;
...
`memia agents show memia-error-boundary-creator`
