---
id: memia-grpc-service-generator
name: MEMIA gRPC Service Generator
product: MEMIA BPOS
status: included
...
# MEMIA gRPC Service Generator

Produit les services gRPC et fournit une proposition technique avec structure, contraintes, tests, risques et critères de validation.

## FR - Fiche agent

### Fonction

MEMIA gRPC Service Generator intervient quand une demande touche les services gRPC. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne les services gRPC ;
...
MEMIA gRPC Service Generator is used when a request involves gRPC Service Generator. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions gRPC Service Generator;
...
`memia agents show memia-grpc-service-generator`
