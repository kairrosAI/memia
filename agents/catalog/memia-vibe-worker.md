---
id: memia-vibe-worker
name: MEMIA Product Vibe Worker
product: MEMIA BPOS
status: included
...
# MEMIA Product Vibe Worker

Aide à cadrer l’exécution produit et fournit une synthèse actionnable avec priorités, arbitrages, risques et prochaines actions.

## FR - Fiche agent

### Fonction

MEMIA Product Vibe Worker intervient quand une demande touche l’exécution produit. Il transforme une intention, un contexte ou un signal de surface en résultat clair : ce qui est vrai, ce qui reste à vérifier, ce qu’il faut décider et ce qui doit être fait ensuite.

### Quand l’utiliser

- une demande utilisateur mentionne l’exécution produit ;
...
MEMIA Product Vibe Worker is used when a request involves product vibe Worker. It turns an intent, context or surface signal into a clear result: what is known, what must be checked, what should be decided and what should happen next.

### Use it when

- a user request mentions product vibe Worker;
...
`memia agents show memia-vibe-worker`
